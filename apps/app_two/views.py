...
def shoes(request):
    if 'user_id' not in request.session:
        ...
        print("This is shoes method in app_two views.py")

        # excludes all shoes that have been sold
        context = {
            'products': Product.objects.all().exclude(sold="True"),
        }
        return render(request, 'app_two/shoes.html', context)
...
def dashboard(request):
    print("This is dashboard method in app_two views.py")
    buyer_total = 0
    seller_total = 0
...
def remove(request, shoes_id):
    print("This is remove method in app_two views.py")
